...
Python 3.11 · FastAPI (async) · Pydantic · sentence-transformers (`alekseevpavel04/multilingual-e5-small-ru-law`, дообученная e5-small; базовая `intfloat/multilingual-e5-small` переключается в конфиге) · Qdrant / FAISS (переключаются в конфиге) · любой OpenAI-совместимый LLM API (облако, Ollama, llama.cpp) · Docker Compose · pytest
...
## Дообученная модель: было и стало

Эмбеддер сервиса дообучен в отдельном исследовании [ru-law-retrieval](https://github.com/alekseevpavel04/ru-law-retrieval): та же `multilingual-e5-small`, дообученная на синтетических вопросах к шести кодексам с дистилляцией знаний из `ai-forever/FRIDA`. Модель лежит на Hugging Face: [alekseevpavel04/multilingual-e5-small-ru-law](https://huggingface.co/alekseevpavel04/multilingual-e5-small-ru-law), теперь она стоит в сервисе по умолчанию.

**Как переключать.** Модель задаётся переменной `EMBEDDING_MODEL` (HF id или локальный путь). У каждой модели свой индекс (`tk_rf` для базовой, `tk_rf__<модель>` для остальных), поэтому векторы разных моделей не смешиваются, а после переключения нужно один раз выполнить `POST /ingest`. Текущая модель видна в `GET /health`. Порог `MIN_SCORE` привязан к модели: у дообученной шкала косинусов уже (обучение с MultipleNegativesRankingLoss растягивает распределение), поэтому старые пороги к ней не переносятся.

**Почему замер важнее переключения.** На родном наборе из 30 вопросов метрики упираются в потолок (Hit@5 = 1.000 у всех dense-моделей), и на нём разницу между моделями не увидеть. Поэтому добавлены два трудных набора из того же исследования:
- [`eval/questions_ru_law_tk.jsonl`](eval/questions_ru_law_tk.jsonl): 74 вопроса по ТК из тестового сплита (17 из них перепроверены вручную);
- [`eval/questions_tk_hard.jsonl`](eval/questions_tk_hard.jsonl): 716 вопросов только двух самых трудных типов (бытовая ситуация и короткий поисковый запрос), по одному на статью, с удалением всего, что похоже на обучающие вопросы.

**Retrieval, FAISS, фрагменты 500/75, без фильтра по главе** (прогоны в [`eval/finetuned/`](eval/finetuned/), парные сравнения с бутстрапом считает [`eval/compare_runs.py`](eval/compare_runs.py)):

| Набор | Метрика | e5-small (было) | дообученная v1 | **дообученная v2 (стало)** |
|---|---|---:|---:|---:|
| 30 вопросов (25 с ответом) | Hit@1 / MRR@5 | 0.960 / 0.980 | 0.880 / 0.933 | **0.960 / 0.980** |
| 74 вопроса по ТК | Hit@1 / MRR@5 | 0.770 / 0.857 | 0.716 / 0.814 | **0.797 / 0.859** |
| 716 трудных вопросов | Hit@1 / MRR@5 | 0.591 / 0.688 | 0.656 / 0.751 | **0.721 / 0.793** |

На трудном наборе Δ MRR@5 = +0.105 с 95% доверительным интервалом [+0.084; +0.127]: по бытовым вопросам +0.128, по поисковым запросам +0.071. На старом наборе изменений нет (потолок), на 74 вопросах прирост Hit@1 есть, но по MRR разница внутри шума (+0.002 [−0.046; +0.048]).

**Генерация** (те же 30 вопросов, LLM и судья `YandexGPT-5-Lite-8B-instruct` через llama.cpp, [`eval/finetuned/full_base_questions.md`](eval/finetuned/full_base_questions.md) и [`full_v2_questions.md`](eval/finetuned/full_v2_questions.md)):

| Метрика | e5-small | дообученная v2 |
|---|---:|---:|
| Ссылка на верную статью | 1.000 (25/25) | 1.000 (25/25) |
| Faithfulness (судья) | 1.000 (25/25) | 1.000 (25/25) |
| Отказ на вопросах без ответа | 0.800 (4/5) | 1.000 (5/5) |
| Ложные отказы | 0.000 (0/25) | 0.000 (0/25) |

Генерация и так упиралась в поиск, а поиск на этом наборе насыщен, поэтому ответы почти не изменились.

**Честная деталь.** Первая версия дообученной модели (v1) в этом сервисе была **хуже** базовой: 0.814 против 0.857 MRR@5 на 74 вопросах. Она выигрывала на общем бенчмарке из шести кодексов за счёт ГК и КоАП, а по ТК прироста не давала. Это выяснилось именно из-за замера «было и стало» до переключения. Вторая версия (учитель FRIDA размечает обучающие данные плюс дистилляция его ранжирования) решила проблему. Скорость и размер индекса не изменились: архитектура та же, 384 измерения, около 22 мс на запрос на CPU.

**Гибридный поиск BM25 + dense не добавлен.** В исследовании вес BM25 подбирался на dev и оказался равен нулю: лексическая часть ничего не добавляет к плотному поиску на этих вопросах.
